...
export function encryptString(stringToEncrypt: string, encryptionKey: string, seed: string): string {
  ...
  const encryptedStringWithIV = Buffer.concat([ivBuffer, Buffer.from(encryptedString, 'hex')]);
  return encryptedStringWithIV.toString('hex');
}

export function decryptString(encryptedString: string, encryptionKey: string, seed: string): string {
